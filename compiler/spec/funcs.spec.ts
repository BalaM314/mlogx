...
describe("getVariablesDefined", () => {
	...
	it("should accept type hints in a set statement", () => {
		expect(getVariablesDefined(
			makeStatement("set thing null", "set thing :building null"),
			commands["set"][0]
		)).toEqual([["thing", "building"]]);
		expect(getVariablesDefined(
			makeStatement("set amogus otherVar", "set amogus :number otherVar"),
			commands["set"][0]
	...
	it("should fail on invalid set statement type hints", () => {
		expect(() => {
			getVariablesDefined(
				makeStatement("set amogus otherVar", "set amogus :sus otherVar"),
				commands["set"][0]
			);
		}).toThrow();
	});

...
export const commands = processCommands({
	...
	//_stackptr points to the first free position on the stack
	//the top item on the stack is at _stackptr - 1
	//this is necessary because the default value of an uninit variable is 0
	//so initializing it to -1 is not necesssary
	push: [{
	...
	jump: [
		{
			...
			port([, addr, op, arg1, arg2], mode){
				if(mode >= PortingMode.shortenSyntax && op == "always") return `jump ${addr}`;
				if(mode >= PortingMode.removeZeroes && op == "always") return `jump ${addr} always`;
	...
	setrule: [
		{
			...
			port([, rule, value, xOrTeam, y, w, h], mode){
				...
				} else if((GenericArgs.get("ruleNormal")!.validator as string[]).includes(rule) && mode >= PortingMode.removeZeroes){
	...
	setprop: [{
		...
		replace(tokens) {
			if(tokens[1].match(/^([\w@_$-()]+?)\.([\w@_$()-]+?)$/i)){
				...
				const [, target, property] = tokens[1].match(/^([\w@_$-()]+?)\.([\w@_$()-]+?)$/i)!;
				if(target == null || property == null) impossible();
				if(!MindustryContent.settables.includes(property)) CompilerError.throw("property not settable", {property});
					...
					return {
						modifiedOutput: compiledOutput.map(line => {
							return new Statement(
								addNamespacesToLine(line.tokens, line.commandDefinitions[0], stack),
								line.sourceText,
								line.cleanedSource.text,

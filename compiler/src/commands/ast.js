...
import { addNamespacesToLine, concat, getCommandDefinition, impossible, isKey, range, splitLineIntoArguments } from "../funcs.js";
        ...
        overloads: [
            {
                ...
                onend({ removedElement, settings, compilerConsts }) {
                    ...
                    for (const el of removedElement.elements) {
                        const { outputProgram, typeCheckingData: outputTypeCheckingData } = compileMlogxToMlog(removedElement.loopBuffer.map(line => line.text), deepmerge(settings, { compilerOptions: { checkTypes: false } }), concat(compilerConsts, [[removedElement.variableName, el]]));
                        compiledCode.push(...outputProgram);
